#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Publish 2026-09-19 Agent 验收瓶颈 article to WeChat draft: cover gen + draft/add + fetch url + backfill"""
...
COVER = '/tmp/cover-review-tax.png'
...
def gen_cover():
    ...
    tf = font(52); sf = font(40); lf = font(22)
    draw.rectangle([60, 78, 160, 82], fill='#ff8c42')
    draw.rectangle([60, 93, 130, 96], fill='#ff8c42')
    draw.text((60, 128), "AI 代码评分更高", fill='#ffffff', font=tf)
    draw.text((60, 208), "生产事故却更多", fill='#ff8c42', font=tf)
    draw.text((60, 300), "验收，成了新瓶颈", fill='#ffffff', font=sf)
    draw.rectangle([60, 388, 260, 391], fill='#ff8c42')
    draw.text((60, 412), "猿来AI · 2026-09-19 深度解读", fill='#8a8a9a', font=lf)
    img.save(COVER)
...
def add_draft(token, thumb):
    ...
    with urllib.request.urlopen(url if False else req, timeout=30) as r:
        d = json.load(r)
    if 'media_id' not in d:
...
def fetch_draft_url(token, media_id):
    ...
    for item in d.get('item', []):
        if item.get('media_id') == media_id:
            ni = item['content']['news_item'][0]
            u = ni.get('url', '')
            if u.startswith('http://'):
                u = 'https://' + u[len('http://'):]
            print("draft url:", u)
            return u
    print("WARN: draft not found in batchget; items:", [i.get('media_id') for i in d.get('item', [])])
    return None
...
if __name__ == '__main__':
    ...
    if url:
        ...
        print("PARTIAL: draft created but url not fetched — run batchget manually")
