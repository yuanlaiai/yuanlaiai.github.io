#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Publish 2026-09-17 Jev/TypeSafe article to WeChat draft: cover gen + draft/add + fetch url + backfill"""
...
def gen_cover():
    ...
    tf = font(54); sf = font(42); lf = font(22)
    draw.rectangle([60, 78, 160, 82], fill='#ff8c42')
    draw.rectangle([60, 93, 130, 96], fill='#ff8c42')
    draw.text((60, 130), "Jev：放弃写字的模型", fill='#ffffff', font=tf)
    draw.text((60, 220), "快 200 倍 · 便宜 444 倍", fill='#ff8c42', font=sf)
    draw.text((60, 300), "不会聊天，却登顶 HN", fill='#ffffff', font=sf)
    draw.rectangle([60, 388, 260, 391], fill='#ff8c42')
    draw.text((60, 412), "猿来AI · 2026-09-17 深度解读", fill='#8a8a9a', font=lf)
    img.save(COVER)
