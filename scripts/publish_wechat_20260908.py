#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Publish 2026-09-08 Mistral article to WeChat draft: cover gen + draft/add + fetch url + backfill"""
import json, os, sys, urllib.request, urllib.parse, uuid, time
...
def gen_cover():
    ...
    tf = font(64); sf = font(42); lf = font(22)
    draw.rectangle([60, 90, 160, 94], fill='#ff8c42')
    draw.rectangle([60, 105, 130, 108], fill='#ff8c42')
    draw.text((60, 145), "三星领投 Mistral €30 亿", fill='#ffffff', font=tf)
    draw.text((60, 240), "欧洲 AI 冠军估值 $240 亿", fill='#ff8c42', font=sf)
    draw.text((60, 330), "主权 AI:地缘叙事还是新生意?", fill='#ffffff', font=sf)
    draw.rectangle([60, 400, 260, 403], fill='#ff8c42')
    draw.text((60, 420), "猿来AI · 2026-09-08 深度解读", fill='#8a8a9a', font=lf)
    img.save(COVER)
